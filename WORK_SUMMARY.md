...
- **Payment Cancellation**: Fixed the "Cancel" button on the Payment Page (`Payment.tsx`). It now correctly cancels the order via `order-service` API instead of just redirecting layout.
...
- **Hard Reset**: Implemented a **Soft Reset** logic (via `hard_reset.py` in Order Service) to:
...
- **Gitignore**: Added `.gitignore` to exclude `node_modules`, `venv`, large files, and the legacy `backend-programlama` folder.

## Technical Changes

### Modified Services
1.  **Order Service** (`app/main.py`):
    - Added `user_map` fetching to resolve names.
    - Added `POST /orders/{id}/cancel` endpoint.
    - Adjusted `get_my_orders` filter.
2.  **Driver Service** (`app/main.py`):
    - Added `lifetime_earnings` calculation.
    - Fixed TypeErrors.
3.  **Frontend**:
    - `Payment.tsx`: Added Cancel logic.
    - `Orders.tsx` / `OrderStatus.tsx`: Added Action Buttons.
    - `TrackDrivers.tsx`: Updated Earnings display.
